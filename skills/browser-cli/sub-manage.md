...
- `~/.browser-cli/` is the user's own git repo — their workflows and tasks, pushable to a remote for cross-device sync.
- `~/.browser-cli-subs/<sub-name>/` is a sibling directory holding clones of **other** people's repos of workflows/tasks. Each sub is its own git checkout, managed by `browser-cli sub *`.
...
- `~/.browser-cli/subs.json` is the registry of subscriptions (tracked in the user's git repo). The dir `~/.browser-cli-subs/` itself is outside the user's repo.
...
The command clones to `~/.browser-cli-subs/<name>/`, updates `subs.json`, and prompts to commit the registry change. Relay the `[y]es / [n]o / [d]iff / [s]how-files` prompt to the user.
...
```bash
browser-cli sub copy <sub-name>/<task>           # copies to ~/.browser-cli/tasks/<task>.ts
...
The command detects whether the source is a workflow or task by where it lives in the sub (`workflows/` vs `tasks/`) and drops the copy into the matching dir under `~/.browser-cli/`. From there, the user can edit it like any other workflow/task and, if desired, PR the fix upstream manually.
...
- **Never edit inside `~/.browser-cli-subs/`.** Those files are overwritten by `sub update`.
- **Never assume a subscribed task is running.** The daemon only schedules tasks in `~/.browser-cli/tasks/`. Subscribed tasks show up in `task list` purely for discoverability, with a `sub copy` hint.
...
Subscriptions travel via `subs.json` — if the user syncs their `~/.browser-cli/` repo across machines (push to a remote), each machine just needs to run `browser-cli sub update` once to re-clone the subs listed in `subs.json` that aren't yet on disk.
