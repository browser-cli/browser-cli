...
- `run` 是一个异步函数，拿到一个开箱即用的 `Stagehand` 实例和解析好的参数，返回任意值。
...
- **零浏览器样板代码。** runner 构造 Stagehand、开 CDP session、把实例交给你。`run` 返回（或抛错）时，runner 只关闭你的 workflow 打开的 page，把原有的 tab 留着，把 Stagehand 关掉。你的代码永远不用 `await browser.close()`。

## Runner 生命周期

runner（在 `src/runner.ts`）是一个薄壳：

1. `loadWorkflow(name)` —— 找到 `~/.browser-cli/workflows/<name>.ts`，转译它，断言模块导出了一个 Zod `schema` 和一个 `run` 函数。
2. `runWorkflow(name, rawArgs)` —— 用 `schema.parse(...)` 解析 `rawArgs`，通过 `makeStagehandConfig` 构造一个 Stagehand 实例，调用 `mod.run(stagehand, parsed)`。
...
workflow 住在 `~/.browser-cli/workflows/<name>.ts`。两条约定要记：

- **文件名就是命令名。** 一个叫 `hn-top.ts` 的文件对应 `browser-cli run hn-top`。
- **home 目录是 git 仓库。** track 你的 workflow、diff 它们、push 它们、作为订阅分享它们。`browser-cli sync` 会帮你把变动 commit 掉。
...
- **Layer 1 —— 拦截网络。** 如果页面从某个 endpoint 拉 JSON，就把它抓下来（`captureResponses`、`waitForJsonResponse`、`pageFetch`）。JSON 结构稳；DOM 都不稳。
- **Layer 2 —— Stagehand 处理 DOM。** 数据或交互只以渲染后的像素存在时，调 `stagehand.act(...)` 或 `stagehand.extract(...)`。selector 会适应漂移；`selfHeal: true` 缓存能跑的结果。
...
有两点值得留意。我们先落到 `github.com` 上，这样后续 `pageFetch` 会继承用户已登录的 session —— GitHub 带鉴权的速率上限比匿名的高一个量级。然后我们彻底跳过 DOM：这个 API 返回 JSON，于是我们用 `pageFetch` 调它然后返回。没有 selector，没有 `extract`，没有 LLM。从头到尾都是 Layer 1。
