...
- **鉴权免费**：`page.fetch` 在页面自己的 JS 上下文里跑，自动继承 cookie 和 origin。
...
2. **页面客户端拉的 JSON 里有你要的数据吗？** → Layer 1（`page.captureResponses` / `page.waitForJsonResponse` / `page.fetch`）。
3. **数据只在 DOM 里渲染？** → Layer 2（用 Zod schema 调 `page.extract`）。
4. **要点击、填写、交互？** → Layer 2（`page.act`）。
